 ...
 */
const mainPageResetAction = StackActions.reset({
    index: 0,
...
const AppStackActions = {
    ...
    resetActionToMainPage: (navigation) => {
        if (navigation != null) {
            navigation.dispatch(mainPageResetAction);
        }
    },
     ...
     * 导航到配置页面
     */
    resetActionToAppSettingPage: (navigation) => {

        const resetAction = StackActions.reset({
            index: 0,
            actions: [NavigationActions.navigate({ routeName: 'AppSetting' })],
        });
